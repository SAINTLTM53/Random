...
function library:init_config(window)
    ...
    -- Capture both subtabs directly (no [2] indexing)
    local mainPage, settingsPage = window:tab({
        name = "Configs",
        tabs = { "Main", "Settings" }
    })

    -----------------------------
    -- MAIN TAB (CONFIG LOGIC)
    -----------------------------
    do
        local column = mainPage:column({})
    ...
    local left = settingsPage:column({})
    local right = settingsPage:column({})

    local teleportSection = left:section({
        name = "Teleport Settings",
        size = 1,
    })

    teleportSection:label({ name = "Made by: Cobra" })
    ...
    serverSection:button({
        ...
        callback = function()
            ...
            if currentPlayers < maxPlayers then
                ...
                msg.Text = "Failed to join server: Server is full."
...
game.RobloxReplicatedStorage.ContactListIrisInviteTeleport:FireServer(game.PlaceId, "", accessCode)
    ...
    local MaxPlayers = "19"
    serverSection:textbox({
        name = "Max Players (default 19)",
        default = MaxPlayers,
        numeric = true,
